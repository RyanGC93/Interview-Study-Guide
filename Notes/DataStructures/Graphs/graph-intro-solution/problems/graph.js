class Graph {
  ...
  buildGraph(edges) { // edges is an array of arrays of vertices
    for (let edge of edges) {  // for each edge in the array of arrays of vertices
      if (edge.length === 1) { // if edge is a single vertex (i.e. a vertex with no neighbors)
  ...
  breadthFirstTraversal(startingVertex) {
    ...
    while(queue.length){ // while there are vertices in the queue (i.e. vertices that have not been visited)
  ...
  depthFirstTraversalIterative(startingVertex) {
    ...
    while (stack.length) {  // while there are vertices in the stack (i.e. vertices that have not been visited)
